# -*- coding: utf-8 -*-
"""
Created on Sun Jul 13 14:23:26 2025
...
class LSTM_attention_v4:
    ...
    def buildfit(self,batch_size=250,n_unit1=128,n_head = 4,epochs=50,callbacks=[BatchLossLogger()]):
    ...
    def plot(self,path='./',i=0,ema_span=21):
        ...
        plt.title(f'Attention4 {self.ticker} pred {self.in_stp}-{self.out_stp}Day {self.trn_rat*100}% prediction')
        ...
        plt.tight_layout()
        plt.savefig(f'{path}LSTM pred att4 {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Loss_plot att4 {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        for h in range(self.num_heads):
            ...
            ax.set_title(f'Att4 {h}-head {self.ticker} weights heatmap')
            ax.set_xlabel('Encoder Time Step')
            ax.set_ylabel('Decoder Time Step')
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Att4_weights_plot {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Gate_plot att4 {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,path='./'):
        with open(f'{path}att4_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.gate.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_q_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.q_dense.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_k_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.k_dense.get_weights(),f)
        with open(f'{path}att4_{self.ticker}_v_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.v_dense.get_weights(),f)
    def load(self,path='./'):
        with open(f'{path}att4_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            gat_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            den_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_q_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            q_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_k_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            k_weights = pickle.load(f)
        with open(f'{path}att4_{self.ticker}_v_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
