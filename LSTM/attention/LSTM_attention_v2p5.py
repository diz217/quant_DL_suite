...
# os.environ['TF_DETERMINISTIC_OPS'] = '1' # is turned on during 0719 run, turned off during 0718 run

class BatchLossLogger(Callback):
    def on_train_batch_end(self, batch, logs=None):
        print(f"\nBatch {batch}: loss = {logs['loss']:.6f}")
        
class LSTM_attention_v2:
    def __init__(self):
    ...
    def buildfit(self,seed = 42, batch_size=250,n_unit1=128,epochs=50,callbacks=[BatchLossLogger()]):
            ...
            for seq in range(seq_max):
                ...
                with tf.GradientTape() as tape: 
                    ...
                    for t in range(self.out_stp):
                        ...
                        score = tf.squeeze(score,axis=2)/math.sqrt(n_unit1)
                        att_weights = tf.nn.softmax(score,axis=1) #batch*time  
                        context_vector = tf.squeeze(tf.matmul(tf.expand_dims(att_weights,axis=1),enc_output),axis=1) #batch*hidden
                ...
                trainable_weights = (self.encoder_lstm.trainable_weights + self.decoder_lstm.trainable_weights +self.gate.trainable_weights+self.decoder_dense.trainable_weights)
    ...
    def my_predict_lite(self,x_encoder_tst):
        ...
        for t in range(self.out_stp):
            ...
            score = tf.squeeze(score,axis=2)/tf.sqrt(d_k)
    ...
    def my_predict(self,x_encoder_tst):
        ...
        return predictions        
    def plot(self,seed = 42, path='./',i=0,ema_span=21):
        ...
        plt.title(f'Attention2 {seed} {self.ticker} pred {self.in_stp}-{self.out_stp}Day {self.trn_rat*100}% prediction')
        ...
        plt.tight_layout()
        plt.savefig(f'{path}LSTM pred att2 {seed} {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        ...
        plt.tight_layout()
        #plt.show(block=False)
        plt.savefig(f'{path}Loss_plot att2 {seed} {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.title(f'Attention2 {seed} {self.ticker} weights heatmap')
        plt.tight_layout()
        #plt.show(block=False)
        plt.savefig(f'{path}Att2_weights_plot {seed} {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.tight_layout()
        #plt.show(block=False)
        plt.savefig(f'{path}Gate_plot att2 {seed} {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,seed=42,path='./'):
        with open(f'{path}att2_{self.ticker}_{seed}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_{seed}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_{seed}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.gate.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_{seed}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
    def load(self,seed=42,path='./'):
        with open(f'{path}att2_{self.ticker}_{seed}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_{seed}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_{seed}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            gat_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_{seed}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
