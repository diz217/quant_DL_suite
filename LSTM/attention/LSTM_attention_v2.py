...
class LSTM_attention_v2:
    ...
    def plot(self,path='./',i=0,ema_span=21):
        ...
        plt.title(f'Attention2 {self.ticker} pred {self.in_stp}-{self.out_stp}Day {self.trn_rat*100}% prediction')
        ...
        plt.tight_layout()
        plt.savefig(f'{path}LSTM pred att2 {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Loss_plot att2 {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.title(f'Attention2 {self.ticker} weights heatmap')
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Att2_weights_plot {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Gate_plot att2 {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,path='./'):
        with open(f'{path}att2_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.gate.get_weights(),f)
        with open(f'{path}att2_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
    def load(self,path='./'):
        with open(f'{path}att2_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_gat_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            gat_weights = pickle.load(f)
        with open(f'{path}att2_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
