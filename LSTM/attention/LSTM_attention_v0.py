# -*- coding: utf-8 -*-
"""
Created on Mon Jul  7 00:29:52 2025
...
class LSTM_attention:
    ...
    def buildfit(self,batch_size=250,n_unit1=128,epochs=50,callbacks=[BatchLossLogger()]):
        ...
        self.attention_dense = Dense(n_unit1)
            ...
            for seq in range(seq_max):
                ...
                with tf.GradientTape() as tape: 
                    ...
                    for t in range(self.out_stp):
                        if np.random.rand()<smpl_prb or t == 0:
                        ...
                        h_enc2 = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1) #batch*hidden
                        h_enc_concat = tf.concat([h_enc2,h_enc],axis=1)#batch*2hidden
                        
                        att_output = self.attention_dense(h_enc_concat)#batch*hidden
                        decoder_output = self.decoder_dense(att_output)#batch*1
    ...
    def my_predict(self,x_encoder_tst):
        ...
        for t in range(self.out_stp):
            _, h, c = self.decoder_lstm(decode_input,initial_state=(h,c)) 
            score = tf.matmul(enc_output,tf.expand_dims(h,axis=2)) 
            score = tf.squeeze(score,axis=2)
            att_weights = tf.nn.softmax(score,axis=1)
            h2 = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1)
            h_concat = tf.concat([h2,h],axis=1)
            att_output = self.attention_dense(h_concat)
            decode_input = self.decoder_dense(att_output)
    ...
    def plot(self,i=0,ema_span=21):
        ...
        plt.title(f'LSTM {self.ticker} 100pred {self.in_stp}-Day {self.trn_rat*100}% prediction')
        ...
        plt.savefig(f'LSTM pred attention {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        plt.show()
        ...
        plt.legend()
        plt.show()
        plt.savefig(f'Loss_plot attention {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,path='./'):
        with open(f'{path}att_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}att_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}att_{self.ticker}_atden_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.attention_dense.get_weights(),f)
        with open(f'{path}att_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
    def load(self,path='./'):
        with open(f'{path}att_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}att_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}att_{self.ticker}_atden_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            atden_weights = pickle.load(f)
        with open(f'{path}att_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
