...
class LSTM_attention_v1:
    ...
    def buildfit(self,batch_size=250,n_unit1=128,epochs=50,callbacks=[BatchLossLogger()]):
        ...
        self.attention_dense = Dense(n_unit1,activation='relu')
        ...
        for epo in range(epochs):
            for seq in range(seq_max):
                ...
                with tf.GradientTape() as tape: 
                    ...
                    for t in range(self.out_stp):
                        if t == 0:
                            ...
                            score = tf.matmul(enc_output,tf.expand_dims(h_enc,axis=2)) #batch*time*1
                            score = tf.squeeze(score,axis=2)
                            att_weights = tf.nn.softmax(score,axis=1) #batch*time                        
                            context_vector = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1) #batch*hidden
                            context_vector = tf.expand_dims(context_vector,axis=1) #batch*1*hidden
                        elif np.random.rand()<smpl_prb:
                            last_close = tf.convert_to_tensor(self.x_decoder[start:end,t:t+1,:],dtype=tf.float32)                            
                        else:
                            last_close = decoder_output
                        proj = self.attention_dense(last_close)
                        cmb_input = tf.concat([proj,context_vector],axis=-1)#batch*1*2hidden
                        decoder_output,h_enc,c_enc = self.decoder_lstm(cmb_input,initial_state=(h_enc,c_enc))
                ...
                trainable_weights = (self.encoder_lstm.trainable_weights + self.decoder_lstm.trainable_weights +self.attention_dense.trainable_weights+self.decoder_dense.trainable_weights)
                ...
                if smpl_prb <1 and epo>=0.8*epochs and seq>=0.7*seq_max and recur_loss.numpy() < best_loss_ever:
                    ...
                    best_att_weights = self.attention_dense.get_weights()
    ...
    def my_predict_lite(self,x_encoder_tst):
        ...
        for t in range(self.out_stp):
            if t==0:
                score = tf.matmul(enc_output,tf.expand_dims(h,axis=2)) 
                score = tf.squeeze(score,axis=2)
                att_weights = tf.nn.softmax(score,axis=1)
                cxt_vec = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1)
                cxt_vec = tf.expand_dims(cxt_vec,axis=1)
            proj = self.attention_dense(decode_input)
            cmb_input = tf.concat([proj,cxt_vec],axis=-1)
            
            decode_input, h, c = self.decoder_lstm(cmb_input,initial_state=(h,c)) 
            score = tf.matmul(enc_output,tf.expand_dims(h,axis=2)) 
            score = tf.squeeze(score,axis=2)
            att_weights = tf.nn.softmax(score,axis=1)
            cxt_vec = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1)
    ...
    def my_predict(self,x_encoder_tst):
        ...
        for t in range(self.out_stp):
            ...
            score = tf.squeeze(score,axis=2)
            att_weights = tf.nn.softmax(score,axis=1)
            att_weights_seq.append(att_weights)
            cxt_vec = tf.reduce_sum(tf.expand_dims(att_weights,axis=2)*enc_output,axis=1)
            cxt_vec = tf.expand_dims(cxt_vec,axis=1)
    ...
    def plot(self,path='./',i=0,ema_span=21):
        ...
        plt.title(f'Attention {self.ticker} pred {self.in_stp}-{self.out_stp}Day {self.trn_rat*100}% prediction')
        ...
        plt.savefig(f'{path}LSTM pred attention1 {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        plt.show()
        ...
        plt.legend()
        plt.show()
        plt.savefig(f'{path}Loss_plot attention1 {self.ticker} {self.in_stp} {self.out_stp}')
        ...
        plt.title(f'Attention {self.ticker} weights heatmap')
        plt.tight_layout()
        plt.show()
        plt.savefig(f'{path}Att1_weights_plot {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,path='./'):
        with open(f'{path}att1_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}att1_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}att1_{self.ticker}_atden_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.attention_dense.get_weights(),f)
        with open(f'{path}att1_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
    def load(self,path='./'):
        with open(f'{path}att1_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}att1_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}att1_{self.ticker}_atden_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            atden_weights = pickle.load(f)
        with open(f'{path}att1_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            den_weights = pickle.load(f)
        self.encoder_lstm.set_weights(enc_weights)
        self.decoder_lstm.set_weights(dec_weights)
        self.attention_dense.set_weights(atden_weights)
