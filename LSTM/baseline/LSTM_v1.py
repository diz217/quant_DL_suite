# -*- coding: utf-8 -*-
"""
Created on Wed Jul  9 01:14:11 2025
...
class LSTM_ssmpl_v2:
            ...
            for seq in range(seq_max):
                ...
                with tf.GradientTape() as tape: 
                    ...
                    for t in range(self.out_stp):
                        ...
                        decoder_output = self.decoder_dense(decoder_output)#batch*1
                ...
                trainable_weights = (self.encoder_lstm.trainable_weights + self.decoder_lstm.trainable_weights+self.decoder_dense.trainable_weights)
                ...
                if smpl_prb <1 and epo>=0.8*epochs and seq>=2 and recur_loss.numpy() < best_loss_ever:
    ...
    def my_predict(self,x_encoder_tst):
        ...
        for t in range(self.out_stp):
            decoder_output, h, c = self.decoder_lstm(decode_input,initial_state=(h,c)) 
            decode_input = self.decoder_dense(decoder_output)
    ...
    def plot(self,path='./',i=0,ema_span=21):
        ...
        plt.title(f'ssmpl2 {self.ticker} pred {self.in_stp}-{self.out_stp} Day {self.trn_rat*100}% prediction')
        ...
        plt.savefig(f'{path}LSTM pred ssmpl_v2 {self.ticker} {self.in_stp}-Day {int(self.out_stp*100)} Panel')
        plt.show()
        ...
        plt.legend()
        plt.show()
        plt.savefig(f'{path}Loss_plot ssmpl2 {self.ticker} {self.in_stp} {self.out_stp}')
    def save(self,path='./'):
        with open(f'{path}ssmpl2_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.encoder_lstm.get_weights(),f)
        with open(f'{path}ssmpl2_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_lstm.get_weights(),f)
        with open(f'{path}ssmpl2_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','wb') as f:
            pickle.dump(self.decoder_dense.get_weights(),f)
    def load(self,path='./'):
        with open(f'{path}ssmpl2_{self.ticker}_enc_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            enc_weights = pickle.load(f)
        with open(f'{path}ssmpl2_{self.ticker}_dec_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
            dec_weights = pickle.load(f)
        with open(f'{path}ssmpl2_{self.ticker}_den_{self.in_stp}Day_{self.out_stp}p{int(self.trn_rat*10)}.keras','rb') as f:
