# -*- coding: utf-8 -*-
"""
Created on Sun Aug  3 23:36:33 2025
...
class TST_v0p1:
    def __init__(self):
        ...
        self.scaler_macd = StandardScaler()  # standard

        self.scaler_bb = MinMaxScaler(feature_range=(0,1)) # min max
        self.scaler_obv = StandardScaler()  # standard
    ...
    def prepare(self,pred_len =60, seq_len=181,trn_rat=0.9):
        ...
        logema = np.log(ema)
        macd = self.dataset[('macd_diff',self.ticker)].values
        rsi = self.dataset[('rsi_14',self.ticker)].values
        bb_band = self.dataset[('bb_bandwidth',self.ticker)].values
        obv = self.dataset[('obv',self.ticker)].values
        ...
        norm_macd = self.scaler_macd.fit_transform(macd.reshape(-1, 1)) # standard
        norm_rsi = (rsi/100).reshape(-1, 1)
        norm_bb = self.scaler_bb.fit_transform(bb_band.reshape(-1, 1)) # min max
        norm_obv = self.scaler_obv.fit_transform(obv.reshape(-1, 1)) # standard
    ...
    def predict(self,seed=0,path='./',i=0,ema_span=21):
        ...
        plt.savefig(f'{path}Transformer8 pred v0 {seed} {self.ticker} {self.seq_len}-{self.pred_len} Panel')
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Loss_plot transformer8 {seed} {self.ticker} {self.seq_len} {self.pred_len}')
        
    def save(self,seed=0, path='./'):
        self.model.save(f'{path}Transformer8_v0_{self.ticker}_{self.seq_len}_{self.pred_len}.keras')
        
        with open(f'{path}Transformer8_history_{self.ticker}_{self.seq_len}_{self.pred_len}.keras','wb') as f:
            pickle.dump(self.history,f)
    def load(self,seed=0, path='./'):
        self.model = load_model(f'{path}Transformer8_v0_{self.ticker}_{self.seq_len}_{self.pred_len}.keras',compile=False,safe_mode=False)
        
        with open(f'{path}Transformer8_history_{self.ticker}_{self.seq_len}_{self.pred_len}.keras','rb') as f:
            self.history = pickle.load(f)
    def compute_importance(self, i=0, seed=0, scoring='neg_mean_squared_error'): 
        feature_cols = ['close', 'ema', 'macd_diff', 'rsi','bb_bandwidth', 'obv', 'ha_body', 'ha_wick_rat']
        ...
        for j,feat_name in enumerate(feature_cols):
            ...
            results.append({'feature': feat_name,'importance_mean': np.mean(deltas),'importance_std': np.std(deltas)})
