...
class BinaryCrossentropyPerColumn(tf.keras.metrics.MeanMetricWrapper):
    def __init__(self, col, name=None):
        def bce_fn(y_true, y_pred):
            y_true_col = y_true[:, col]
            y_pred_col = y_pred[:, col]
            return tf.keras.losses.binary_crossentropy(y_true_col, y_pred_col)
        super().__init__(bce_fn, name=name)
...
class BatchLossLogger(Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        print(f"\n Epoch:{self.epoch}, Batch {batch}: tp_loss = {logs['tp_yield_loss']:.3f}, y_loss = {logs['y_hat_loss']:.3f}, learning = {current_lr:.3e}, Est. time left: {int(mins):02d}:{int(secs):02d}")
...
class TST_v0p8:
    ...
    def prepare(self,pred_len =60, seq_len=181,trn_rat=0.9,stop_loss=0.94):
        ...
        tst_tp = norm_tp[trn_len-seq_len:]
    ...
    def fit(self,batch_size=32, epochs=50):
        ...
        self.history = self.model.fit(self.x_encoder,{'tp_yield':self.y_signal,'y_hat':self.y_target},
                                      validation_data=[self.x_tst[0:10],{'tp_yield':self.y_tsi[0:10],'y_hat':self.y_tst[0:10]}],
    ...
    def predict(self,seed=0,path='./',i=0,ema_span=21):
        ...
        plt.title(f'Price head {seed} {self.ticker} pred {self.seq_len}-{self.pred_len}Day {self.trn_rat*100}% prediction')
        ...
        plt.savefig(f'{path}Transformer6 pred v0p8 {seed} {self.ticker} {self.seq_len}-{self.pred_len} Panel')
        #=========================loss plot==================================
        self.yhat_loss = self.history.history['y_hat_loss']
        self.val_yhat_loss = self.history.history.get('val_y_hat_loss',None)
        self.tps_loss = self.history.history.get('tp_yield_loss',None)
        self.val_tps_loss = self.history.history.get('val_tp_yield_loss',None)
        ...
        axes[1].set_ylabel('Tp yield Loss')
        axes[1].legend()
        axes[1].grid(True)
        
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}Loss_plot transformer6 v0p8 {seed} {self.ticker} {self.seq_len} {self.pred_len}')         
        #=========================TP yield plot==================================
        ...
        plt.tight_layout()
        plt.show(block=False)
        plt.savefig(f'{path}TP_yield_plot transformer6 v0p8 {seed} {self.ticker} {self.seq_len} {self.pred_len}')         
